...
plugin_info_t Plugin_info = {
	...
	"The Legion",			// author
	"",						// url
	"REUNION",				// logtag, all caps please
...
C_DLLEXPORT int Meta_Attach(PLUG_LOADTIME now, META_FUNCTIONS *pFunctionTable, meta_globals_t *pMGlobals, gamedll_funcs_t *pGamedllFuncs)
{
	...
	return Reunion_Load() ? TRUE : FALSE;
}
...
bool Reunion_Load() {
	if (!Reunion_Utils_Init())
		return false;

	if (!Reunion_Cfg_LoadDefault())
		return false;

	if (!Reunion_RehldsApi_Init()) {
		LCPrintf(true, "Failed to locate REHLDS API\n");
		return false;
	}

	if (!Reunion_Init_Players()) {
		LCPrintf(true, "Failed to initialize players\n");
		return false;
	}

	Reunion_Init_Authorizers();
	Reunion_Init_ServerInfo();

	if (!Reunion_Auth_Init()) {
		LCPrintf(true, "Reunion_Auth_Init failed\n");
	}

...
IF EXIST "%srcdir%\version.h" (
	FOR /F "usebackq tokens=1,2,3" %%i in ("%srcdir%\version.h") do (
		IF %%i==#define (
			...
			IF %%j==VERSION_SUFFIX set version_suffix=%%~k
		)
	)
)
...
set new_version=%version_major%.%version_minor%.%version_maintenance%.%commitCount%%version_suffix%
...
echo #define APP_VERSION_INT %version_major%%version_minor%%version_maintenance%%commitCount%>>"%srcdir%\appversion.h"
echo #define APP_VERSION_STRD "%version_major%.%version_minor%.%version_maintenance%.%commitCount%%version_suffix%">>"%srcdir%\appversion.h"
